...
public class ContentCategorySumBQSubscriber {
	...
	public static void main(String[] args) {
		...
		System.out.println("Pipleline is created");
		...
		System.out.println("SumByCategoryTypeComposite is completed");
		sumByCategoryType.apply(BigQueryIO.<KV<Long, Long>>write().to("sitevisits-195700:sitevisits.SUM_BY_CONTENT_CAT_TYPE").withSchema(new TableSchema().setFields(
			         ImmutableList.of(
				 ...
				 .withFormatFunction(quote -> new TableRow().set("USER_NAME", "porumamilla_raghu").set("CAT_TYPE", quote.getKey()).set("NUM_MINUTES_SPENT", quote.getValue()).set("DATE", getDate(new Date())).set("UPDATED_TIME_STAMP", getTimestamp()))			     
			     .withWriteDisposition(BigQueryIO.Write.WriteDisposition.WRITE_TRUNCATE));
		/*sumByCategoryType
				.apply(MapElements.via(new FormatAsTextFn()))
				.apply(TextIO.write().to(options.getOutput()));*/
		System.out.println("Saved to big query is completed");
